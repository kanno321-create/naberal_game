"""Deep Research runner — 6 분할 쿼리 순차 실행.

대표님 NotebookLM (`a0bb9e88-b8cc-4a8d-a55a-75ffa01996eb`) 의 소스 자료에 근거하여
초보 1인 인디 게임 개발자 + 나베랄 감마 AI 공동 학습용 전 영역 지식을 획득.

Pacing (대표님 지시 2026-04-20):
- 각 쿼리 timeout 420s (7분) — "15분까지 필요없다"
- 쿼리 사이 sleep 10s — Playwright 세션 안정화
- 총 예상 30-40분 (6 쿼리 순차)

D-6 single-string 규율: 쿼리 6개는 pre-composed 단일 문자열. argv 로 각 1회 전달.
결과: `.planning/research/nlm_result_0{1..6}_*.md` 에 저장.

사용법:
    cd studios/game/
    python scripts/notebooklm/run_deep_research.py           # 6 쿼리 전부
...
TIMEOUT_S = 420  # 7 min per query — 대표님 "15분까지 필요없다" 2026-04-20
SLEEP_BETWEEN = 10  # Playwright 세션 안정화


QUERIES: list[tuple[str, str, str]] = [
    (
        "01_indie_mvp_design",
        "영역 1-3: 인디 현실 + MVP + 게임 디자인 원칙",
    ...
    (
        "02_unity6_all",
        "영역 Unity 6 LTS 전부 (로드맵 + 스크립팅 + 비주얼 + 퍼포먼스)",
    ...
    (
        "06_mental_legal_ai",
        "영역 프로젝트 관리/멘털 + 법률(한국) + AI 시대 2026",
...
def main() -> int:
    ...
    if len(sys.argv) > 1:
        try:
            target_indices = [int(x) - 1 for x in sys.argv[1:]]
        except ValueError:
            print("Usage: run_deep_research.py [query_index...]  (1-based)", file=sys.stderr)
            return 2

    studio_root = Path(__file__).resolve().parents[2]
    out_dir = studio_root / ".planning" / "research"
    out_dir.mkdir(parents=True, exist_ok=True)

    total_start = time.time()
    completed = 0
    for idx, (stem, label, question) in enumerate(QUERIES):
        if target_indices is not None and idx not in target_indices:
            continue
        out_path = out_dir / f"nlm_result_{stem}.md"
        if out_path.exists() and target_indices is None:
            print(f"[{idx + 1}/{len(QUERIES)}] SKIP (already exists): {out_path.name}")
            completed += 1
            continue

        print(f"[{idx + 1}/{len(QUERIES)}] START: {label}")
        ...
        try:
            ...
            return 1
        dt = time.time() - t0
        print(f"    DONE in {dt:.1f}s, answer length: {len(answer)} chars")
        ...
        if idx + 1 < len(QUERIES):
            still_has_next = target_indices is None or any(i > idx for i in target_indices)
            if still_has_next:
                print(f"    sleep {SLEEP_BETWEEN}s before next query...")
                time.sleep(SLEEP_BETWEEN)

    total_dt = time.time() - total_start
    print(f"\nCompleted {completed} queries in {total_dt:.1f}s total")
    return 0
