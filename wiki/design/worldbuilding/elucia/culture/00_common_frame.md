---
title: "Elucia 문화 공통 프레임 — 인덱스"
layer: 2
canon_tier: detail
tags: [worldbuilding, elucia, culture]
updated: 2026-04-22
parent: "[[culture_00_common_frame]]"
moc: "[[elucia_MOC]]"
derived_from:
  - "[[story_full_narrative]]"
  - "[[brainstorm_2026-04-21_worldview_expansion]]"
canon_anchors:
  - src: "[[brainstorm_2026-04-21_worldview_expansion]]"
    quote: "서쪽 대륙: 서구중세문명 — 왕국·기사·교회·봉건제 구조"
  - src: "[[game_setting_complete_2026-04-21]]"
    quote: "인간은 악이 아니다. 모든 종족이 나태해진 신과 타락한 교회의 피해자"
related:
  - "[[economy_00_overview]]"
  - "[[political_00_overview]]"
  - "[[history_00_timeline]]"
agent_briefing_level: reference
type: culture
subject: index
created: 2026-04-22
agent: Wave2-Culturalist
wave: 2
status: COMPLETE — 17파일 전체 작성 완료 (2026-04-22)
inputs:   - wiki/design/brainstorm_2026-04-21_worldview_expansion.md
  - wiki/design/game_setting_complete_2026-04-21.md
  - wiki/design/political_divisions.md
  - wiki/design/worldbuilding/elucia/geography/climate_zones_2026-04-22.md
  - .claude/agents/worldbuilding/_shared_briefing.md
qcore_version: v1.0
---

# Elucia 문화 공통 프레임 — 인덱스 (00_common_frame)

> **이 파일은 네비게이션 인덱스 전용이다. 내용 서술 금지.**
> Wave 2 Culturalist 산출물 전체의 진입점. Wave 4 Kingdom-Detailer 가 왕국 고유 문화 확장 시 이 인덱스를 기준으로 variation 축을 참조한다.

---

## 원전 인용 증명

### [필독 1] brainstorm_2026-04-21_worldview_expansion.md:143–145 (발언 3)
> "서쪽 대륙: 서구중세문명 / 동쪽 대륙: 이슬람류문명 사막이 대부분이다"
— 발언 3, brainstorm_2026-04-21_worldview_expansion.md:143–145

### [필독 2] brainstorm_2026-04-21_worldview_expansion.md:261–267 (발언 7)
> "좌우 대륙은 같은 신을 믿지만 서로 해석을 달리한다. 서로 적대적이긴하나 하나의 목표는 지성이있는 타 종족 몰살로 오로지 인류를 위한 행성을 목표로한다."
— 발언 7, brainstorm_2026-04-21_worldview_expansion.md:261–267

### [필독 3] brainstorm_2026-04-21_worldview_expansion.md:337–358 (발언 9·10)
> "엘프는 이쁘장한 외모로 노예시장에서 많이 팔리고, 드워프는 뛰어난 손기술로 노예로 많이 팔린다. / 오크는 투기장에서 놀이감으로 사용되며, 마족은 오로지 사냥을 당한다"
— 발언 9·10, brainstorm_2026-04-21_worldview_expansion.md:337–409

### [필독 4] .claude/agents/worldbuilding/_shared_briefing.md:47–52 (Q-CORE 2)
> "할배는 자신의 수많은 판단 미스로 많은 생명 앗은 것을 속죄 / 현재 인간의 삶 (정체 완전 숨김) / 생활 마법 개발·무료 배포 (가난한 서민 수혜)"
— _shared_briefing.md:47–52 (Q-CORE 2 확정)

### [필독 5] game_setting_complete_2026-04-21.md:64–87 (세계관 철학 3조)
> "가. 불완전성 — 모든 존재는 완벽하지 않다 / 나. 나이트 인격체 원칙 / 다. 영혼 평등 — 모든 영혼은 크기가 다를 수 있으나 고귀함은 같다"
— game_setting_complete_2026-04-21.md:64–87

### [필독 6] brainstorm_2026-04-21_worldview_expansion.md:2776–2813 (발언 46·47·48)
> "어느마을이나 교회가있다. 농업위주의 삶, 서쪽은 징병제, 동쪽은 모병제"
> "동쪽은 농업 어업, 서쪽은 농업 축산업"
— 발언 46·48, brainstorm_2026-04-21_worldview_expansion.md:2776–2912

### [필독 7] brainstorm_2026-04-21_worldview_expansion.md:2930–2942 (발언 49)
> "동쪽이 워낙 지형이 험하고 혹독하여 성격이 조금더 거칠고, 잔인하며, 노예시장도 발달함"
— 발언 49, brainstorm_2026-04-21_worldview_expansion.md:2930–2942

---

## 요약

Elucia (서쪽 대륙) 의 문화는 발언 3 "서구중세문명" 을 기반으로 한다. 왕국·기사·교회·봉건제가 사회 골격을 이루며, 발언 7 의 종교 구조(같은 신 · 해석 차이) 가 문화 전반의 이데올로기 기반을 형성한다. Wave 2 Culturalist 는 11 왕국 + 교황청 제국에 공통으로 적용되는 **통일 프레임** 과 **variation 축** 을 정립한다. 왕국별 고유 의상·축제·음식·건축 세부는 Wave 4 Kingdom-Detailer 가 확장한다.

---

## 파일 목록 (Wave 2 · 2026-04-22 생성)

| 파일 | 주제 | 핵심 키워드 |
|------|------|------------|
| [[shared_religion_structure_2026-04-22|`shared_religion_structure_2026-04-22.md`]] | 공통 종교 구조 (같은 신) | 7대 신·수직 교계·신앙 기능 |
| [[religious_schism_orthodox_vs_corrupt_2026-04-22|`religious_schism_orthodox_vs_corrupt_2026-04-22.md`]] | 타락한 교회 vs 양심파 분열 | 교리 분열·지역 분포·양심파 저항 |
| [[common_language_base_2026-04-22|`common_language_base_2026-04-22.md`]] | 대륙 공용어 Elucian | 문자·기본 문법·라틴 어원 |
| [[dialects_regional_framework_2026-04-22|`dialects_regional_framework_2026-04-22.md`]] | 지역별 방언 variation 프레임 | 기후·지형별 발음·어휘 차이 |
| [[medieval_fantasy_conventions_2026-04-22|`medieval_fantasy_conventions_2026-04-22.md`]] | 중세 판타지 공통 설정 | 봉건제·기사도·교단·세금 |
| [[clothing_framework_2026-04-22|`clothing_framework_2026-04-22.md`]] | 의상 공통 프레임 | 기후·신분별 variation 축 |
| [[festivals_common_seasonal_2026-04-22|`festivals_common_seasonal_2026-04-22.md`]] | 계절 축제 공통 | 수확제·동지제·봄맞이 |
| [[festivals_religious_common_2026-04-22|`festivals_religious_common_2026-04-22.md`]] | 종교 축제 공통 | 성인기념일·성지 순례 |
| [[cuisine_regional_framework_2026-04-22|`cuisine_regional_framework_2026-04-22.md`]] | 기후별 음식 variation 프레임 | 서쪽 축산·동쪽 어업 |
| [[architecture_styles_framework_2026-04-22|`architecture_styles_framework_2026-04-22.md`]] | 건축 양식 공통 프레임 | 고딕·로마네스크·목조+석조 |
| [[social_classes_2026-04-22|`social_classes_2026-04-22.md`]] | 신분 계급 | 왕족·귀족·성직·시민·농노·노예 |
| [[coming_of_age_rite_common_2026-04-22|`coming_of_age_rite_common_2026-04-22.md`]] | 성인식 공통 프레임 | 의지결 목걸이·교회 의식 |
| [[folklore_nameless_scholar_2026-04-22|`folklore_nameless_scholar_2026-04-22.md`]] | 구전 "이름 없는 학자" 전설 | Q-CORE 2 간접 단서 |
| [[magic_in_daily_life_2026-04-22|`magic_in_daily_life_2026-04-22.md`]] | 서민 일상 속 마법 | 할배 무료 생활 마법 6종 |
| [[literacy_and_education_2026-04-22|`literacy_and_education_2026-04-22.md`]] | 문해율·교육 체계 | 교회학교·귀족 가정교사·마법대학 |
| [[funeral_and_burial_2026-04-22|`funeral_and_burial_2026-04-22.md`]] | 장례·매장 관습 | 지역별 변이·귀족 석관·평민 공동묘지 |
| [[marriage_and_family_2026-04-22|`marriage_and_family_2026-04-22.md`]] | 결혼·가족 구조 | 교회 혼인 성사·봉건 혼약·대가족 |

---

## Variation 축 요약 (Wave 4 참조용)

Wave 4 Kingdom-Detailer 는 아래 variation 축을 기준으로 각 왕국의 고유 문화를 작성한다.

| 축 | 공통 프레임 | 변이 방향 |
|----|-----------|---------|
| **기후** | 6개 기후대 (climate_zones 참조) | 해양성 → 목초·안개 문화 / 고산 → 광업·방한 문화 |
| **산업** | 농업+축산업 (Elucia 공통) | 해안 왕국 → 어업 보조 / 내륙 → 소·말 중심 |
| **교회 영향도** | 모든 마을 교회 존재 | 성좌국 직할 → 최강 / 변경 왕국 → 상대적 약 |
| **봉건 밀도** | 왕→공작→백작→남작→기사→평민 | 제국권 → 층위 촘촘 / 변경 → 단순화 |
| **타종족 접촉** | 숲·습지·가장자리에 은신 | 가장자리 왕국 → 접촉 빈번 / 중앙 → 희소 |
| **방언** | Elucian 공용어 기반 | 해안 → 항해 어휘 / 산악 → 광업 어휘 |

---

## Wave 의존 포인트

| Wave | 에이전트 | 이 파일에서 참조할 내용 |
|------|---------|----------------------|
| Wave 3 | Historian | 문화 기원·변천사 기술 시 공통 프레임 참조 |
| Wave 3 | Diplomat | 왕국 간 관계에서 문화 충돌 기술 |
| Wave 4 | Kingdom-Detailer × 12 | variation 축 기준으로 왕국 고유 문화 확장 |
| Wave 5 | Chronicler | 역사서·연대기에 문화 흔적 기술 |

---

*인덱스 생성: 2026-04-22 · Wave2-Culturalist · naberal_game*
