---
title: "Copperholt — Greygate 구리 채굴 전문 마을"
type: village
kingdom: kingdom_thaloss
created: 2026-04-22
updated: 2026-04-22
agent: Wave4-Kingdom-Detailer-thaloss
wave: 4
---

# Copperholt (코퍼홀트)

| 항목 | 내용 |
|------|------|
| **위치** | Greygate 서사면 구리맥 지대 |
| **인구** | 약 500~800 |
| **산업** | 구리 채굴·구리 괴 1차 정련 |
| **어원** | *copper* (구리) + *-holt* (숲·둘레, 게르만) = "구리 숲 마을" |
| **분위기** | 녹청색 먼지가 덮임 · 가옥 외벽이 구리색으로 변색 |
| **특징** | Varn 공작 직속 관리 — 왕실 세금보다 공작 세금이 더 높음 |

<!-- auto-generated-related:start -->
## 🔗 관련 (auto-generated)

> `scripts/obsidian/build_backlinks.py` 로 자동 생성. 수정 금지 — 다음 실행 시 덮어쓰여집니다.

### ⬆️ 상위

- [[../../../../../../MOC]] — wiki 루트
- [[../../../MOC]] — Elucia 허브

### 👑 왕국 허브

- [[../../MOC]] — 10 왕국 + 성좌국

### 🏛️ 소속 왕국 영토

- [[../../../political/kingdom_thaloss_territories_2026-04-22]]

### 🏘️ 형제 마을

- [[village_ashgate_2026-04-22]]
- [[village_ashvein_2026-04-22]]
- [[village_coldthorn_2026-04-22]]

<!-- auto-generated-related:end -->
