---
title: "Saltmarsh — 서남 해안 소금 마을"
layer: 2
canon_tier: detail
tags: [worldbuilding, elucia, kingdom_aldric, villages]
updated: 2026-04-22
kingdom: kingdom_aldric
parent: "[[kingdom_aldric_00_overview]]"
moc: "[[../../../../design/MOC]]"
derived_from:
  - "[[story_full_narrative]]"
  - "[[brainstorm_2026-04-21_worldview_expansion]]"
canon_anchors:
  - src: "[[kingdom_aldric_territories_2026-04-22]]"
    quote: "Duchy of Coastfen / 해안 어업·소금"
  - src: "[[story_full_narrative]]"
    quote: "불완전성 — 모든 존재는 완벽하지 않다"
related:
  - "[[duke_coastfen_selyne_2026-04-22]]"
  - "[[road_lonwyn_to_coastfen_2026-04-22]]"
  - "[[city_westport_aldric_2026-04-22]]"
agent_briefing_level: reference
type: village
created: 2026-04-22
agent: Wave4-Kingdom-Detailer-kingdom_aldric
wave: 4
qcore_version: v1.0
---

## 원전 인용 증명

### [필독 1] kingdom_aldric_territories_2026-04-22.md
> "Duchy of Coastfen / 해안 어업·소금"

### [필독 2] 에이전트 지시 — 의상
> "린넨+가벼운 가죽"

---

## 요약

Coastfen 공작령 서남 해안 소금 생산 마을. 해수를 증발시켜 소금을 만드는 염전이 마을 경제의 핵심이다.

---

## 기본 정보

| 항목 | 내용 |
|------|------|
| **위치** | Aldric 서남 해안 Coastfen 공작령 내 |
| **인구** | 약 300~450 |
| **산업** | 해염 생산·해안 어업 |
| **어원** | *salt* (소금) + *marsh* (습지·갯벌) |
| **분위기** | 해풍. 하얀 소금 더미. 강인한 해안 사람들 |

---

## 대표님 미확정
- 알드릭 소금 교역 범위 (Ceren·성좌국 수출 여부)

## 다음 Wave 의존
- Wave 5 World-Integrator: 소금 교역로 통합
