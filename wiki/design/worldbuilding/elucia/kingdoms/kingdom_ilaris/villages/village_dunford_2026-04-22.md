---
title: "Dunford — Ilaris 내륙 여울 마을"
layer: 2
canon_tier: detail
tags: [worldbuilding, elucia, kingdom_ilaris, villages]
updated: 2026-04-22
kingdom: "kingdom_ilaris"
parent: "[[kingdom_ilaris/00_overview]]"
moc: "[[../../../../design/MOC]]"
derived_from:
  - "[[story_full_narrative]]"
  - "[[brainstorm_2026-04-21_worldview_expansion]]"
canon_anchors:
  - src: "[[political_divisions]]:110"
    quote: "Silvan / 실반 / 서해안 숲 / 일라리스 왕국"
  - src: "[[political_divisions]]:55"
    quote: "일라리스 / Ilaris / 서해안"
  - src: "[[story_full_narrative]]"
    quote: "세계관 철학 3조 — 불완전성·한결같음·영혼 평등"
related:
  - "[[kingdom_ilaris/00_overview]]"
  - "[[kingdom_ilaris/capital_map_2026-04-22]]"
  - "[[political_divisions]]"
agent_briefing_level: reference
type: village
tier: village
created: 2026-04-22
agent: Wave2-Toponymist
wave: 2
---

# Dunford (던포드) — Silvan 권역 강변 여울 마을

| 항목 | 내용 |
|------|------|
| **위치** | Eloryn 강 상류 지류 여울 · Caerveld 남방 |
| **인구** | 약 300~500 |
| **기능** | 강 도하 지점 · 소규모 수력 방아 |
| **어원** | *dun* (언덕 요새, L3 켈트) + *-ford* (여울) |
| **분위기** | 조용한 강변 마을. 이따금 귀족 기마대가 지나감 |
