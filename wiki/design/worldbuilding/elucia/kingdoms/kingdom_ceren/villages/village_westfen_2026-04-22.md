---
title: "Westfen — Ceren 서해안 어촌"
layer: 2
canon_tier: detail
tags: [worldbuilding, elucia, kingdom_ceren, villages]
updated: 2026-04-22
kingdom: kingdom_ceren
parent: "[[kingdom_ceren_00_overview]]"
moc: "[[../../../../design/MOC]]"
derived_from:
  - "[[story_full_narrative]]"
  - "[[brainstorm_2026-04-21_worldview_expansion]]"
canon_anchors:
  - src: "[[brainstorm_2026-04-21_worldview_expansion]]"
    quote: "좌측은 강이 많고 풍요로움"
  - src: "[[political_divisions]]:111"
    quote: "Loravel / 로라벨 / 서남 습지·호수 / 세렌 왕국"
related:
  - "[[duke_deltawatch_mossway_2026-04-22]]"
  - "[[city_irondelta_south_2026-04-22]]"
  - "[[road_fenlyn_to_ironstrand_2026-04-22]]"
agent_briefing_level: reference
type: village
created: 2026-04-22
agent: Wave4-Kingdom-Detailer-ceren
wave: 4
---

# Westfen (웨스트펜)

| 항목 | 내용 |
|------|------|
| 위치 | Deltawatch 공작령 서해안 · Auravel 삼각주 서단 |
| 인구 | 약 600~1,000 |
| 산업 | 해안 어업 · 소량 소금 · 서해 교역 환적 |
| 어원 | *west* (서쪽) + *fen* (습지) |
| 특징 | 세렌 왕국 최서단 정착지 · Ironstrand 와 해안 경쟁 |
| 분위기 | 서해 바람 거셈 · 소금기 가득한 공기 · 작은 방파제 |
| 행정 | Westfen 백작령 소속 |
