---
title: "Elorbrook — Eloryn 강 연어 마을"
layer: 2
canon_tier: detail
tags: [worldbuilding, elucia, kingdom_vaelin, villages]
updated: 2026-04-22
kingdom: "kingdom_vaelin"
parent: "[[kingdom_vaelin/00_overview]]"
moc: "[[../../../../design/MOC]]"
derived_from:
  - "[[story_full_narrative]]"
  - "[[brainstorm_2026-04-21_worldview_expansion]]"
canon_anchors:
  - src: "[[political_divisions]]:109"
    quote: "Havren / 하브렌 / 북서 해안 / 모란·바엘린 왕국"
  - src: "[[political_divisions]]:53"
    quote: "바엘린 / Vaelin / 북부 평원"
  - src: "[[story_full_narrative]]"
    quote: "세계관 철학 3조 — 불완전성·한결같음·영혼 평등"
related:
  - "[[kingdom_vaelin/00_overview]]"
  - "[[kingdom_vaelin/capital_map_2026-04-22]]"
  - "[[political_divisions]]"
agent_briefing_level: reference
type: village
created: 2026-04-22
agent: Wave4-Kingdom-Detailer-vaelin
wave: 4
---

# Elorbrook (엘로르브룩) — Eloryn 강 연어잡이 마을

| 항목 | 내용 |
|------|------|
| 위치 | Elorfeld 공작령 · Eloryn 강 상류 지류 |
| 인구 | 약 300~400 |
| 산업 | 연어 포획 · 훈제 연어 가공 · 수운 보조 |
| 어원 | Eloryn(강 이름) + Brook(시내) |
| 분위기 | 물소리 · 연기 냄새 · 계절 어촌 |
| 특징 | 봄 연어 소상 시즌 = 마을 최대 행사. 훈제 연어는 Vaelthorn까지 수출 |
