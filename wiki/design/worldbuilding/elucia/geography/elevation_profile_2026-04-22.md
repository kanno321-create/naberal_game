...
```mermaid
graph LR
    A[Norvend 고산 4200m] -->|서쪽 사면 하강| B[Aurion 평야 200m]
    B -->|강 풍요| C[농업·성좌국·Solaris]
    A -->|동쪽 사면 완만| D[Orenwald 구릉 500m]
    D -->|짧은 하천| E[Oryn 왕국 숲 경계]
    B -->|남쪽 흐름| F[Soranth 평원 100m]
    F -->|Azim Pass 방향| G[남부 지협 연결]
    A -->|북서 사면| H[Havren 해안 구릉]
    H -->|모란·바엘린 왕국| I[북서 해안 항구]
```
