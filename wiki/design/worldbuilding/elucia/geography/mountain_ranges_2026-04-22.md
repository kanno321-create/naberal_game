...
```mermaid
graph TD
    VG[Veilglass 북쪽 얼음섬] -->|남쪽| NR[Norvend Range 주맥 동서 900km]
    NR -->|서쪽 끝| MC[Morncliff Spine 북서 구릉]
    NR -->|동쪽 끝| GR[Veilorn Ridge 동쪽 경계 700km]
    NR -->|서쪽 사면| RIV[서쪽 대하천 발원]
    NR -->|고개 Greygate| TH[Thaloss 왕국 남부]
    MC -->|해안 절벽| MO[Moran·Vaelin 왕국]
    GR -->|서쪽 사면 숲| OR[Orenwald 동부 숲]
    GR -->|동쪽| KZ[Karzor 방향 건조 기류 차단]
    TH -->|남쪽 평야| AU[Aurion Divide 중부 구릉]
    AU -->|제국 동경계| SOL[Solaris 성좌국 영역]
```
