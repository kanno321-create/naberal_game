...
```mermaid
graph LR
    GR[Veilorn Ridge 동쪽 방벽] -->|자연 동벽| OR[Oryn 왕국]
    OW[Orenwald 숲] -->|국토 대부분| OR
    SO[Soranth 강 발원] -->|서쪽 수운| OR
    OR -->|숲 접경| MA[Maerith 왕국]
    OR -->|평원 접경| CH[성좌국]
    OR -->|평원 접경| SY[Sylren 왕국]
    OR -->|숲 접경| NO[Novas 왕국]
```
